...
func TestExecuteCpuLoop(t *testing.T) {
	...
	require.NoError(t, err)
	requireOkResponse(t, res, 0)
	assert.Equal(t, uint64(0xddcab790), cost)
	t.Logf("Time (%d gas): %s\n", cost, diff)
...
func TestMultipleInstances(t *testing.T) {
	...
	assert.Equal(t, uint64(0x135e02f8c), cost)
	...
	resp := exec(t, cache, checksum, "mary", store1, api, querier, 0x11f1a8900)
	require.Equal(t, "Unauthorized", resp.Err)

	// succeed to execute store1 with fred
	resp = exec(t, cache, checksum, "fred", store1, api, querier, 0x21f1367a0)
	...
	resp = exec(t, cache, checksum, "mary", store2, api, querier, 0x21f939420)
	require.Equal(t, "", resp.Err)
	require.Equal(t, 1, len(resp.Ok.Messages))
	attributes = resp.Ok.Attributes
	require.Equal(t, 2, len(attributes))
