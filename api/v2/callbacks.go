...
func recoverPanic(ret *C.GoError) {
	if rec := recover(); rec != nil {
		...
		default:
			log.Printf("Panic in Go callback: %#v1\n", rec)
			debug.PrintStack()
			*ret = C.GoError_Panic
		}
	}
...
func BuildDBState(kv utils.KVStore, callID uint64) DBState {
	return DBState{
		Store:  kv,
		CallID: callID,
	}
...
func BuildDB(state *DBState, gm *utils.GasMeter) C.Db {
	return C.Db{
...
func cGet(ptr *C.db_t, gasMeter *C.gas_meter_t, usedGas *Cu64, key C.U8SliceView, val *C.UnmanagedVector, errOut *C.UnmanagedVector) (ret C.GoError) {
	...
	gm := *(*utils.GasMeter)(unsafe.Pointer(gasMeter))
	kv := *(*utils.KVStore)(unsafe.Pointer(ptr))
	k := CopyU8Slice(key)
	...
	*usedGas = (Cu64)(gasAfter - gasBefore)
	...
	// v1 will equal nil when the key is missing
...
// since this is only used internally, we can verify the code that this is the case
func BuildAPI(api *utils.GoAPI) C.GoApi {
	return C.GoApi{
...
func cHumanAddress(ptr *C.api_t, src C.U8SliceView, dest *C.UnmanagedVector, errOut *C.UnmanagedVector, used_gas *Cu64) (ret C.GoError) {
	...
	api := (*utils.GoAPI)(unsafe.Pointer(ptr))
	s := CopyU8Slice(src)
...
// since this is only used internally, we can verify the code that this is the case
func BuildQuerier(q *types.Querier) C.GoQuerier {
	return C.GoQuerier{
...
func cQueryExternal(ptr *C.querier_t, gasLimit C.uint64_t, usedGas *C.uint64_t, request C.U8SliceView, result *C.UnmanagedVector, errOut *C.UnmanagedVector) (ret C.GoError) {
	...
	// query the data
	querier := *(*types.Querier)(unsafe.Pointer(ptr))
